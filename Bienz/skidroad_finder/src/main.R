...
# Holen der Argumente von der Kommandozeile
args <- commandArgs(trailingOnly = TRUE)

if (length(args) < 2) {
  stop("Bitte gib beide Pfade als Argumente an: <path_ground> <path_delineation>")
}

# Setze die Pfade aus den Argumenten
path_ground <- args[1]
path_delineation <- args[2]

# Überprüfe, ob die angegebenen Pfade existieren
if (!file.exists(path_ground)) {
  stop("Der angegebene Pfad zum Ground-Structure-Dataset existiert nicht.")
}

if (!file.exists(path_delineation)) {
  stop("Der angegebene Pfad zur Forest-Delineation-Datei existiert nicht.")
}

print(paste("Ground Structure Dataset:", path_ground))
print(paste("Forest Delineation:", path_delineation))
